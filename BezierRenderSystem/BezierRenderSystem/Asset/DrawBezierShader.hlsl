	...
	float4 color : COLOR) : SV_TARGET
{
    ...
    float C = dot(coefficient1.xy, project.xy) * 2 + coefficient0.z;
    float D = dot(coefficient1.zw, project.xy) + coefficient0.w;
    ...
    if (root0 >= 0 && root0 <= 1.0f){
        ...
        if (distance(project.xy, Q) <= coefficient2.z) return color;
    }
