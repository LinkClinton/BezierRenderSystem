...
StructuredBuffer<Equation> equations : register(t1);
...
float msaa_sample(float2 position, float alpha, uint id) 
{
    ...
    float4 coefficient0 = equations[id].coefficient0;
    float4 coefficient1 = equations[id].coefficient1;
    float4 coefficient2 = equations[id].coefficient2;
    ...
    uint instance_id : SV_InstanceID) 
{
	...
	result.projection = mul(float4(position, 1.0f), transforms[instance_id].world);
	result.projection = mul(result.projection, transforms[instance_id].projection);
    ...
    uint instance_id : SV_InstanceID) : SV_TARGET
{
	...
	if (msaa_sample(center, color.a, instance_id) == 0) discard;

	float sample0 = msaa_sample(center + float2(0.25f, 0.25f), color.a, instance_id);
	float sample1 = msaa_sample(center + float2(0.25f, -0.25f), color.a, instance_id);
	float sample2 = msaa_sample(center + float2(-0.25f, 0.25f), color.a, instance_id);
	float sample3 = msaa_sample(center + float2(-0.25f, -0.25f), color.a, instance_id);
